...
function RuntimeInspector({ settings, contextPercent, document, selectedSkill }) {
  ...
  return (
    <div className="runtime-inspector">
      <header><strong>Runtime Inspector</strong><span>Deterministic evidence</span></header>
      <dl>
        <div><dt>Runtime</dt><dd>{selected.label} · {selected.status}</dd></div>
        <div><dt>Model</dt><dd>{settings.provider.model || "Unavailable"}</dd></div>
        <div><dt>Context</dt><dd>{contextPercent}% · exact mock count</dd></div>
      ...
      <div className="inspector-diagnostic">No real process, network request, credential value, or hidden reasoning is exposed.</div>
    </div>
  );
}
...
function ThreadHistory({ threads, activeId, showArchived, setShowArchived, onSelect, onNew, onRename, onArchive, onDelete, onClose, running }) {
  ...
  return (
    <div className="agent-history">
      <header><strong>Threads</strong><button type="button" onClick={onClose}><X size={14} /></button></header>
      <button className="history-new" type="button" onClick={onNew}><SquarePen size={13} />New Thread</button>
      <label className="history-filter"><input type="checkbox" checked={showArchived} onChange={(event) => setShowArchived(event.target.checked)} />Show archived</label>
      <div className="history-list">
        {visible.map((thread) => (
          <div className={`history-row ${thread.id === activeId ? "is-active" : ""}`} key={thread.id}>
            <button className="history-main" type="button" onClick={() => onSelect(thread.id)}><strong>{thread.title}</strong><small>{thread.archived ? "Archived" : `${thread.items.length} items`}</small></button>
            <div>
              <button type="button" title="Rename Thread" onClick={() => onRename(thread.id)}><RefreshCw size={12} /></button>
              <button type="button" title={thread.archived ? "Restore Thread" : "Archive Thread"} onClick={() => onArchive(thread.id)}><Archive size={12} /></button>
              <button type="button" title="Delete Thread" disabled={running && thread.id === activeId} onClick={() => onDelete(thread.id)}><Trash2 size={12} /></button>
            </div>
          </div>
        ))}
      </div>
...
function AgentCrown({ onNew, onHistory, onInspector, onClose }) {
  return (
    <header className="agent-crown" data-tauri-drag-region>
      <div className="agent-heading"><span className="agent-mark"><Sparkles size={15} /></span><strong>Agent</strong></div>
      <div className="inline-actions agent-header-actions">
        {onNew && <button className="icon-button" type="button" aria-label="New thread" onClick={onNew}><SquarePen size={15} /></button>}
        {onHistory && <button className="icon-button" type="button" aria-label="Thread history" onClick={onHistory}><History size={15} /></button>}
        {onInspector && <button className="icon-button" type="button" aria-label="Runtime Inspector" onClick={onInspector}><Info size={15} /></button>}
...
export function AgentPanel({ document, settings, editorAdapter, onOpenSettings, onToggleAgent }) {
  ...
  const runPrompt = async (prompt) => {
    ...
    if (!settings.provider.credentialConfigured) {
      appendItem({ type: "error", text: "Configure a mock Provider credential in Settings before starting a Turn." });
      return;
    }
  ...
  if (!settings.provider.credentialConfigured) {
    return (
      <aside className="agent-region" aria-label="Agent">
        <AgentCrown onClose={onToggleAgent} />
  ...
  return (
    <aside className="agent-region" aria-label="Agent">
      <AgentCrown onNew={newThread} onHistory={() => setHistoryOpen((value) => !value)} onInspector={() => setInspectorOpen((value) => !value)} onClose={onToggleAgent} />
      <div className="agent-thread-title"><strong>{activeThread.title}</strong><span>{activeThread.items.length} items</span></div>
      ...
      {inspectorOpen && <RuntimeInspector settings={settings} contextPercent={contextPercent} document={document} selectedSkill={selectedSkill} />}
      {historyOpen && (
        <ThreadHistory
          threads={threads}
          activeId={activeId}
          ...
          onNew={newThread}
          running={running}
          onRename={(id) => setThreads((items) => items.map((thread) => thread.id === id ? { ...thread, title: `${thread.title} · revised` } : thread))}
          onArchive={(id) => setThreads((items) => items.map((thread) => thread.id === id ? { ...thread, archived: !thread.archived } : thread))}
          onDelete={(id) => {
