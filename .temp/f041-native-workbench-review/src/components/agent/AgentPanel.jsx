...
import { mockAgentRuntime, runtimeCatalog } from "../../mock/mockAgentRuntime.js";
...
function RuntimeInspector({ settings, contextPercent, document, selectedSkill }) {
  const selected = runtimeCatalog.find((runtime) => runtime.selected);
...
export function AgentPanel({ document, settings, editorAdapter, onOpenSettings }) {
  ...
  return <aside className="agent-region" aria-label="Agent"><header className="agent-crown" data-tauri-drag-region><div className="agent-heading"><span className="agent-mark"><Sparkles size={15} /></span><span><strong>Agent</strong><small>{running ? "Turn running" : "Codex mock runtime"}</small></span></div><div className="inline-actions"><button className="icon-button" type="button" aria-label="New thread" onClick={newThread}><SquarePen size={15} /></button><button className="icon-button" type="button" aria-label="Thread history" onClick={() => setHistoryOpen((value) => !value)}><History size={15} /></button><button className="icon-button" type="button" aria-label="Runtime Inspector" onClick={() => setInspectorOpen((value) => !value)}><Info size={15} /></button></div></header><div className="agent-thread-title"><strong>{activeThread.title}</strong><span>{activeThread.items.length} items</span></div><div className="agent-scope"><span>Attached document</span><strong>{document.name}</strong></div>{contextPercent >= settings.agent.exactContextWarning && <button className="context-warning" type="button" onClick={newThread}>Context is {contextPercent}%. Start a New Thread for exact edits.</button>}<div className="agent-thread" aria-live="polite">{activeThread.items.length ? activeThread.items.map((item) => <TranscriptItem key={item.id} item={item} showTools={settings.agent.showToolActivity} />) : <div className="agent-empty agent-empty--thread"><Bot size={22} /><strong>Work with the active editor</strong><p>Ask for an outline, request a bounded edit, or type “fail” to review terminal recovery.</p><div><button type="button" onClick={() => runPrompt("Outline the active document structure")}>Outline structure</button><button type="button" onClick={() => runPrompt("Edit the current selection with a concise review note")}>Apply a native edit</button></div></div>}</div>{inspectorOpen && <RuntimeInspector settings={settings} contextPercent={contextPercent} document={document} selectedSkill={selectedSkill} />}{historyOpen && <ThreadHistory threads={threads} activeId={activeId} showArchived={showArchived} setShowArchived={setShowArchived} onClose={() => setHistoryOpen(false)} onSelect={(id) => { setActiveId(id); setHistoryOpen(false); }} onNew={newThread} running={running} onRename={(id) => setThreads((items) => items.map((thread) => thread.id === id ? { ...thread, title: `${thread.title} · revised` } : thread))} onArchive={(id) => setThreads((items) => items.map((thread) => thread.id === id ? { ...thread, archived: !thread.archived } : thread))} onDelete={(id) => { const remaining = threads.filter((thread) => thread.id !== id); if (remaining.length) { setThreads(remaining); if (id === activeId) setActiveId(remaining[0].id); } else { const replacement = mockAgentRuntime.createThread(document); setThreads([replacement]); setActiveId(replacement.id); } }} />}<div className="agent-composer"><div className="composer-skill-row"><select aria-label="Agent Skill" value={selectedSkill} onChange={(event) => setSelectedSkill(event.target.value)} disabled={running}><option value="">Automatic Skill</option>{availableSkills.map((skill) => <option key={skill.id} value={skill.name}>{skill.name}</option>)}</select><span>{settings.agent.deliveryMode}</span></div><div className="composer-box"><textarea aria-label="Ask Agent" rows={3} value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask about the active document…" onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); running ? steer() : runPrompt(input); } }} /><div className="composer-foot"><span>{running ? "Enter steers this Turn" : "Mock Runtime · active editor tools"}</span><div>{activeThread.items.some((item) => item.type === "error") && !running && <button className="composer-retry" type="button" aria-label="Retry last Turn" onClick={() => runPrompt(lastPromptRef.current)}><RefreshCw size={13} /></button>}{running && <button className="composer-cancel" type="button" aria-label="Cancel Turn" onClick={cancel}><CircleStop size={15} /></button>}<button type="button" aria-label={running ? "Steer Turn" : "Send message"} onClick={() => running ? steer() : runPrompt(input)} disabled={!input.trim()}>{running ? <MoreHorizontal size={15} /> : <Send size={15} />}</button></div></div></div></div></aside>;
}
