...
function AgentSettings({ draft, setDraft }) {
  ...
  return <section className="settings-section"><span className="settings-kicker">Agent</span><h2>Runtime and policy</h2><p>Automatic selection prefers the healthy mock Codex runtime and shows compatibility fallback evidence.</p><div className="runtime-list">{runtimeCatalog.map((runtime) => <div className={`runtime-row ${runtime.selected ? "is-selected" : ""}`} key={runtime.id}><span className={`runtime-dot runtime-dot--${runtime.status}`} /><div><strong>{runtime.label}</strong><small>{runtime.capabilities.join(" · ")}</small></div><span>{runtime.selected ? "Selected" : runtime.status}</span></div>)}</div><Field label="Maximum Tool steps"><input type="number" min="2" max="40" value={agent.maxSteps} onChange={(event) => update({ maxSteps: Math.max(2, Math.min(40, Number(event.target.value))) })} /></Field><Field label="New Thread warning" description="Recommend a fresh Thread after this context percentage."><input type="number" min="50" max="95" value={agent.exactContextWarning} onChange={(event) => update({ exactContextWarning: Math.max(50, Math.min(95, Number(event.target.value))) })} /></Field><Field label="Answer delivery"><select value={agent.deliveryMode} onChange={(event) => update({ deliveryMode: event.target.value })}><option value="incremental">Incremental</option><option value="burst">Burst</option><option value="atomic">Atomic</option></select></Field><Field label="Tool Activity"><Toggle label="Show Tool Activity" checked={agent.showToolActivity} onChange={(value) => update({ showToolActivity: value })} /></Field><button className="settings-secondary" type="button" onClick={() => update({ maxSteps: 12, exactContextWarning: 78, showToolActivity: true, deliveryMode: "incremental", diagnostics: 20, replayEvents: 200 })}><RefreshCw size={13} />Reset Agent policy</button></section>;
}
...
function SkillSettings({ draft, setDraft }) {
  ...
  return <section className="settings-section settings-section--wide"><span className="settings-kicker">Skills</span><h2>Managed Agent Skills</h2><p>Skills provide instructions and selection metadata. They cannot widen editor Tool capabilities.</p><div className="skills-list">{draft.skills.map((skill) => <div className="skill-row" key={skill.id}><span className={`skill-source skill-source--${skill.source}`}>{skill.source}</span><div><strong>{skill.name}</strong><small>{skill.path ?? `Built into ${skill.scope === "all" ? "the application" : skill.scope}`}</small></div><select aria-label={`Scope for ${skill.name}`} value={skill.scope} onChange={(event) => updateSkill(skill.id, { scope: event.target.value })}><option value="all">All editors</option><option value="ideasketch">IdeaSketch</option><option value="markdown">Markdown</option></select><Toggle label={`Enable ${skill.name}`} checked={skill.enabled} onChange={(value) => updateSkill(skill.id, { enabled: value })} />{skill.source === "custom" && <button type="button" aria-label={`Remove ${skill.name}`} onClick={() => setDraft({ ...draft, skills: draft.skills.filter((item) => item.id !== skill.id) })}><Trash2 size={14} /></button>}</div>)}</div><button className="settings-secondary" type="button" disabled={busy} onClick={async () => { setBusy(true); const skill = await mockSettingsApi.importSkill(); setDraft({ ...draft, skills: [...draft.skills, skill] }); setBusy(false); }}><Plus size={13} />Import mock Skill folder</button><div className="settings-callout"><ShieldCheck size={18} /><span><strong>Validation is simulated</strong><small>Use Review Scenarios later to inspect an invalid custom Skill.</small></span></div></section>;
}
...
export function SettingsCenter({ settings, onSettings, onTheme, onClose }) {
  ...
  return <div className="dialog-backdrop"><section className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title"><header className="settings-header"><div><strong id="settings-title">Settings</strong><small>Application preferences · deterministic mock</small></div><div className="settings-header-actions">{status === "saved" && <span><Check size={12} />Saved</span>}{status === "error" && <span className="text-danger">Save failed</span>}<button className="settings-save" type="button" onClick={save} disabled={status === "saving"}>{status === "saving" ? "Saving…" : "Save changes"}</button><button className="icon-button" type="button" onClick={onClose} aria-label="Close Settings"><X size={16} /></button></div></header><div className="settings-layout"><nav className="settings-nav" aria-label="Settings categories">{sections.map(([id, label]) => <button className={section === id ? "is-selected" : ""} type="button" key={id} onClick={() => setSection(id)}>{label}</button>)}</nav><div className="settings-content">{section === "general" && <GeneralSettings draft={draft} setDraft={setDraft} onTheme={onTheme} />}{section === "provider" && <ProviderSettings draft={draft} setDraft={setDraft} />}{section === "agent" && <AgentSettings draft={draft} setDraft={setDraft} />}{section === "skills" && <SkillSettings draft={draft} setDraft={setDraft} />}{section === "ideasketch" && <IdeaSketchSettings draft={draft} setDraft={setDraft} />}</div></div></section></div>;
}
