...
- In the right rail, show the current camera as a larger highlighted card at the top.
- Show the remaining cameras as smaller scrollable cards below it.
...
- Top section: one larger "current camera" card with strong active emphasis.
- Lower section: smaller camera cards in a scrollable list.
...
- The selected camera gets a larger card pinned at the top of the rail.
- This card should make it obvious which camera the user is actively working with.
- Remaining cameras stay visible below as smaller cards.
- This asymmetry is intentional and preferred over giving every camera equal visual weight.
...
- The selected camera appears in the larger pinned card region.
- The remaining cameras continue to appear in the list section.
...
- If the currently selected camera is deleted:
  - the larger pinned card area should update immediately
...
- Confirm the current camera is shown as a larger highlighted card and remaining cameras appear as smaller scrollable cards.
...
- Add focused source-level or state-level coverage for:
  ...
  - current-camera emphasis wiring
...
- `CameraList.tsx` will likely need to split into:
  - current camera card treatment
  - remaining camera card list treatment
