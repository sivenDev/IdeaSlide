...
- [x] Prove a signed update round trip or stop with the exact external secret/tag authorization still required; do not claim completion from UI mocks alone.
...
- [x] After the authorized remote round trip succeeds, mark F056 done/complete and refresh the index in a follow-up task commit.
...
- B044 closed the remaining native acceptance with a disposable locally signed installed-client `0.2.7` to `0.2.8` round trip outside the production application: the old process installed the update and exited, LaunchServices started a distinct new process from the replaced bundle, and both runtime and bundle reported `0.2.8`. Save / Discard / Cancel and duplicate-operation behavior remain covered by the shared lifecycle regressions.
