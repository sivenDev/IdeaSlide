...
## F039: Explore workspace-first AI agent product layouts
...
## F045: Refine review demo Workspace actions and document chrome

- status: done
- created: 2026-08-11

Refine only .temp/f041-native-workbench-review after human review. Remove creation controls from the Workspaces section header. Give every Workspace root a trailing New (+) control and overflow menu; give directories the same controls; files receive only the overflow menu. New creates only inside the selected Workspace root or directory, while overflow actions cover rename, move where valid, reveal-in-Finder simulation, remove or Trash with safe confirmation. Keep Open Workspace available outside the section header through the command palette and empty state. Recents must contain only standalone files opened outside a Workspace; opening a Workspace or a Workspace-owned file must never add a Recent. Fix the closed-Workspace grid so Editor Host remains visible and receives the released width. Redesign the document crown by removing the dedicated Save button, Close button, editor-owner explanation, and separate status rail; add one accessible leading status/close control before the file icon and title that shows clean/dirty/saving/error state at rest and becomes Close on hover or focus, while autosave and Command/Ctrl+S remain functional. Simplify Agent chrome by removing explanatory subtitle/first-use feature copy, right-aligning New Thread, History, Inspector, and placing the open-panel close toggle at the far right; keep the closed robot toggle context-gated in Editor Host. Preserve English copy, Light/Dark/System, responsive resizing, mock-only platform boundaries, deterministic scenarios, and the no-production-migration boundary.
...
## F062: Define Enacta V2 as an Agent-First Personal Colleague

- status: done
- created: 2026-08-14

Create `docs/product/enacta-product-guide-v2.md` as a Chinese V2 product guide that upgrades V1 from a workspace-first AI tool into an Enacta Agent-first personal colleague. On launch, text and voice conversation with a persistent Agent are primary; Workspace, Markdown, IdeaSketch, other editors, and generated Web views open only when data needs human-readable visualization, collaborative ideation, review, or direct editing. Treat durable user data, memory, permissions, tasks, decisions, provenance, and outcomes as the product core while UI remains a replaceable projection. Define the Agent Home, voice/text interaction, memory and data model, permission dialogs, editor/view orchestration, generated Web fallback, Workspace entry, outcome lifecycle, local-first trust boundaries, V1 migration, roadmap, metrics, risks, non-goals, and representative scenarios such as sketching a report outline in IdeaSketch before the Agent produces an HTML report. Support multiple role-based Enacta personas, such as Work, Life, and user-defined roles, over one user identity and one governed memory foundation rather than separate data silos. Personas may differ in tone, goals, tools, permissions, and memory lens, but cross-persona retrieval must respect scope, relevance, sensitivity, and explicit grants. Define transparent memory importance, confidence, provenance, recency, repetition, correction, pinning, decay, compaction, expiration, contextual forgetting, and explicit deletion so low-value conversations can fade while important verified memories persist under user control. Prioritize the roadmap by weighted user expectations rather than novelty: reliable outcome completion, trust and control, low-friction frequent interaction, and memory compounding precede broad personas, proactive autonomy, ambient desktop-pet behavior, and an expansive generated-UI or plugin ecosystem. Each phase must state its user-value weight, prerequisites, observable success gate, and the evidence required before advancing. Preserve real files as the authority for user artifacts, editor-native transactions and Undo/Redo, external-change protection, capability-based permissions, visible/auditable actions, and the Enacta Agent Kernel decision in RFC 003. Use the linked article `从“工具”到“同事”——AI 时代的产品进化` as product inspiration without copying it.

## F063: Sharpen Enacta V2 First-Use and MVP Product Strategy
