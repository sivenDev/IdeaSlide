...
test('Toolbar keeps generic file commands and centered IdeaNote document title', async () => {
  ...
  const editor = await readSource('src/components/EditorLayout.tsx');
  const workspaceStart = await readSource('src/components/WorkspaceStart.tsx');
  ...
  assert.doesNotMatch(source, /<House |Back to Home|onGoHome/);
  ...
  assert.match(editor, /\bhandleNewFile\b/);
  assert.match(editor, /onNewFile=\{handleNewFile\}/);
  assert.match(workspaceStart, /onNewFile\("ideasketch"\)/);
  assert.match(workspaceStart, /onNewFile\("markdown"\)/);
  assert.match(workspaceStart, /New IdeaSketch/);
  ...
  assert.match(styles, /--idea-slide-danger:\s*var\(--ideanote-danger\)/i);
